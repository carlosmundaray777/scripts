...
function autofillp_smatv(
    ...
    var repuesta1 = JSON.parse(
        typeof respuestas == "object" ? JSON.stringify(respuestas) : respuestas
    );
    ...
    if (trato_especial(pregunta) == trato_especial(query2)) {
        if (repuesta1 != undefined) {
            for (var k = 0; k < Object.keys(repuesta1).length; k++) {
                let inputs = wrapper.querySelectorAll("input, textarea");
                inputs.forEach((input, i2) => {
                    if (i2 == repuesta1[k].position) {
                        switch (repuesta1[k].type) {
                            case "text":
                            case "textarea":
                            case "hidden":
                                input.value = repuesta1[k].value;
                                break;
                            case "radio":
                            case "checkbox":
                                input.click();
