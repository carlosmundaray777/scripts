    ...
    configureSet((form,submissions,keywords) => {
          ...
          const submissions_api = query => {
            ...
            jsawesome.forEach(wrapper => {
                ...
                let code = name?.replace("Find the Leverage for this ISIN","")?.trim();
...
function scraping(wrapper, code){
                    ...
                    GM_xmlhttpRequest({
                        method: "GET",
                            url: "https://mdweb.ngm.se/MDWebFront/quotes.html?activeTab=searchall&locale=us_en&hideNavigationBar=&search="+code,
                                    headers:    {
                                        ...
                                        onload: function(response) {
                                            ...
                                            let is_existe=buscarCode(response.responseText,"div", "quotesDiv");

                                            console.log(response.responseText);

                                            /*if (!response.finalUrl.toString().toLowerCase().includes("products/?q=")) {
                                               setTimeout(function () {
                                                wrapper.style.backgroundColor = "navy";
                                                var link = document.createElement("a");
                                                link.innerText = "Ver LEVIER";
                                                        ...
                                                        url: response.finalUrl,
                                                                headers:    {
                                                                    ...
                                                                    onload: function(r) {
                                                                        ...
                                                                        var buscador="Leverage";
                                                                        let num_is=buscarCode(cadena, "Leverage").match(/(\d+)/g);
                                                                        let num=buscarCode(cadena, "Leverage")[0].replace(/[<>td/]/g, '');
...
function buscarCode(cadena, etiqueta , buscador){
    let invertido = cadena.split(etiqueta).reverse().join(etiqueta);
    return invertido.substring(invertido.indexOf(buscador), invertido.length).split(etiqueta)[0];
}
