...
setTime(10000,13000);
//id de la guia
identifier = '1-JFLDHJG9E5723432B4JM';
limit = 100;
          ...
          const keyword = query => {
            ...
            jsawesome.forEach(wrapper => {
                ...
                if(p1.includes(p2) && p2!=""){
                    ...
                    var radios=wrapper.querySelectorAll(".radios.cml_field");
                    var product_details=document.getElementsByName(wrapper.id+"[product_details]");
                    var seller_details=document.getElementsByName(wrapper.id+"[seller_details]");
                    setTimeout(function tiempo(){
                       ...
                       //console.log(product_details);
                        product_details.forEach(tq =>{
                            if(!tq.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.getAttribute("class").includes("_cf_hidden")){
                                    var r1=trato_especial_s(tq.value,5);
                                    var r2=trato_especial_s(Object.values(fill[0]?.content)[2],5);
                                    if(r1==r2 && r2!=""){
                                        if(tq.type=="radio"){tq.checked=1}
                                        tq.click();
                                    }
                            }
                        });

                        seller_details.forEach(tq =>{
                            if(!tq.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.getAttribute("class").includes("_cf_hidden")){
                                    var r1=trato_especial_s(tq.value,5);
                                    var r2=trato_especial_s(Object.values(fill[0]?.content)[3],5);
                                    if(r1==r2 && r2!=""){
                                        if(tq.type=="radio"){tq.checked=1}
                                        tq.click();
                                    }
                    ...
                    if(mode.toLowerCase().includes("work")){
                        ...
                        if(typeof(keywordfill[0])!='undefined') {
                            ...
                            setTimeout(function tiempo(){
                             var idd= wrapper.id;
                             var primero = document.getElementsByName(idd+"[review_content]");
                             primero[keywordfill[0].respuesta-1].checked=1;
                             primero[keywordfill[0].respuesta-1].click();
