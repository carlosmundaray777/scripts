    ...
    configureSet((form,submissions,keywords) => {
          ...
          const submissions_api = query => {
            ...
            jsawesome.forEach(wrapper => {
                ...
                let code = name?.replace("Find the Partizipationsfacktor for this ISIN","")?.trim();
...
function scraping(wrapper, code){
                    ...
                    GM_xmlhttpRequest({
                        method: "GET",
                            url: "https://www.wallstreet-online.de/suche/?suche=&q="+code+"&sa=Suche",
                                    headers:    {
                                        ...
                                        onload: function(response) {
                                            ...
                                            if (response.finalUrl.toString().toLowerCase().includes("zertifikat")) {
                                               setTimeout(function () {
                                                ...
                                                let regex = /(\d+)/g;
                                                let numero=buscarCode(response.finalUrl, "00x");

                                                console.log(response.finalUrl);
                                                console.log(numero);

                                               document.getElementsByName(wrapper.id+"[does_the_website_sow_a_partizipationsfaktor]")[0].click();
                                               document.getElementsByName(wrapper.id+"[please_enter_the_partizipationsfaktor]")[0].value=numero;
                                               },1000);
                                            }else{
...
function buscarCode(cadena, buscador){
    ...
    return invertido.substring(invertido.indexOf(buscador), invertido.length).split("-")[1];
}
