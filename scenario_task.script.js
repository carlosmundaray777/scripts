...
setTime(20000,20000);
//id de la guia
identifier = '1-MBF3MH1263GC1HHMHJ4E';
    ...
    configureSet((form,submissions,keywords) => {
          ...
          const keyword = query => {
            ...
            jsawesome.forEach(wrapper => {
                ...
                var name = wrapper.querySelectorAll("span")[0]?.textContent;
                ...
                var element_rs = wrapper.querySelectorAll("input[type=text]");
                element_rs.forEach(element => {
                  console.log(element);
                    element.classList.remove("validation-passed");
                    element.classList.remove("validates-unique");
                    element.classList.remove("validates-multiple_rows_unique");
                    element.classList.remove("validates-coherence");
                    element.classList.remove("single-field-validation-failed");
                    element.classList.remove("validation-failed");
                    element.classList.remove("validates-lang");
                });
                ...
                if(p2.includes(p1) && p2!=""){
                    ...
                    let respuestas=Object.values(fill[0]?.content)[1].split(">");
                    let campos = wrapper.querySelectorAll('input[type=text]');


    for (var r = 0; r < respuestas.length; r++) {
      for (var i = 0; i < campos.length; i++) {
        let input=campos[r];
        if (input!=null) {campos[r].value=respuestas[r];}
        }
    }
