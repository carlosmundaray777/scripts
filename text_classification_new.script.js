...
setTime(20000,25000);
//id de la guia
identifier = '1-HJLE7HK2MECCAC2L54J4';

idguia=796874;

var ruta="div > div.span6.centered > div";
      ...
      requestData().then(json => {
            ...
            setTimeout(function () {
                ...
                if(mode.includes("work") && tq_found?.length >= 1){
                    ...
                    //para que es esto carlos?
                    //para sacar la cantida de tarea que lleva porque joiner me dijo que solo 2 tarea podia ahacer
                    var cantidad_task=document.querySelector(".completed-gauge")?.textContent.match(/(\d+)/g)[0];
                    console.log(cantidad_task);
                  if (cantidad_task <= 4) {

                        tq_for_update_no_existe.forEach(wrapper=>{
                            if (!wrapper.classList.contains('tq_found')) {
                                wrapper.style.backgroundColor = "silver";
                                document.getElementsByName(wrapper.id+"[is_the_potential_classification_correct]")[1].click();
                            }    
                        });
