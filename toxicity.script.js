    ...
    configureSet((form,submissions,keywords) => {
        ...
        const keyword = query => {
            ...
            jsawesome.forEach(wrapper => {
                ...
                var keywordfill = keyword({'name':name?.replace("Please read the following text carefully:","").trim()});
            ...
            jsawesome.forEach(wrapper => {
                ...
                for(var i=0; i<resp?.length; i++){
                    ...
                    if (!wrapper.classList.contains('tq_found')) {
                        autofillppp(wrapper,ruta,atributo,r,color);
                    }
                }
            });
    ...
    if (trato_especial(pregunta) == trato_especial(query)) {
                ...
                case "textarea":
                    ...
                    for (var i = 0; i < textareaall.length; i++) {
                        ...
                        if (textareaall[i].tipo == "TEXTAREA") {
                            inp.value = String(palabra);
                            textarea_respuesta=String(textareaall[i].valor);
                            inp.focus();
                            inp.click();
                            existe = true;
                        }
        ...
        if (existe) {
            ...
            wrapper.classList.add("tq_found");
                            ...
                            var name1 = pregunta;
                            let data=[{"tq_id":wrapper.id,"data":[name1 , select_respuesta,textarea_respuesta]}];
